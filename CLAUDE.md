...
**Đọc song ngữ:** file nhập theo đoạn (`paragraphs: Array<Array<{ en, vi }>>`), server tính `order` (liên tục qua mọi đoạn), `paragraphIndex` (vị trí đoạn), `wordCount` trong `src/features/reading/import-reading.ts`. Trang đọc là lưới theo đoạn (`md:grid-cols-2`), không có JS cuộn đồng bộ — hai cột tự thẳng hàng. Mỗi câu tiếng Anh là `<span data-translate-context order={order}>` để popup dịch lấy câu làm ngữ cảnh (sửa quy tắc `blockContext()` trong `src/components/translate-popup/selection-utils.ts`). Cột tiếng Việt có `data-no-translate` để popup không hiện khi bôi đen tiếng Việt. `getReading()` chỉ trả bài `PUBLISHED`. Trang `[id]` gộp hai lần gọi (metadata + dữ liệu) bằng React `cache()`.
